package tydic.framework.starter.mq.kafka;
...
import tydic.framework.starter.mq.TydicMQProperties;
...
public class KafkaConsumers implements DisposableBean {
    ...
    private final Map<String, TydicMQProperties.ConsumerProperties> consumerProperties = new HashMap<>();
    ;
    ...
    public synchronized KafkaConsumer<String, byte[]> create(String source) {
        ...
        TydicMQProperties.ConsumerProperties consumerProperties = this.consumerProperties.get(source);
    ...
    public void init(TydicMQProperties properties) {
        ...
        for (String source : enable) {
            TydicMQProperties.ConsumerProperties consumerProperties = properties.getKafkaConsumer().get(source);
            if (consumerProperties == null) {
                throw CacheBuilderException.of("无法获取kafka消费者配置:{}", source);
            }
